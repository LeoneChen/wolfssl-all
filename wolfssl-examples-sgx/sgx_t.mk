...
trusted/Wolfssl_Enclave_t.c: $(SGX_EDGER8R) ./trusted/Wolfssl_Enclave.edl
	@cd ./trusted && $(SGX_EDGER8R) --trusted ../trusted/Wolfssl_Enclave.edl --search-path ../trusted --search-path $(SGX_SDK)/include --search-path $(SGXSanPath)/SGXSanRT/SGXSanRTEnclave
	@echo "GEN  =>  $@"

trusted/Wolfssl_Enclave_t.o: ./trusted/Wolfssl_Enclave_t.c $(SGXSan_Pass)
#	@$(CC) $(Wolfssl_Enclave_C_Flags) -c $< -o $@
#	@$(CLANG) -Wstrict-prototypes $(Wolfssl_Enclave_C_Flags) -S -emit-llvm -c $< -o ${@:%.o=%.pre.ll}
	@$(CLANG) -Wstrict-prototypes $(Wolfssl_Enclave_C_Flags) -Xclang -load -Xclang $(SGXSan_Pass) -mllvm -sgxsan-at-enclave-tbridge=true -c $< -o $@
#	@$(CLANG) -Wstrict-prototypes $(Wolfssl_Enclave_C_Flags) -Xclang -load -Xclang $(SGXSan_Pass) -mllvm -sgxsan-at-enclave-tbridge=true -S -emit-llvm -c $< -o ${@:%.o=%.ll}
	@echo "CC   <=  $<"

trusted/%.o: trusted/%.c
#	@$(CC) $(Wolfssl_Enclave_C_Flags) -c $< -o $@
#	@$(CLANG) -Wstrict-prototypes $(Wolfssl_Enclave_C_Flags) -S -emit-llvm -c $< -o ${@:%.o=%.pre.ll}
	@$(CLANG) -Wstrict-prototypes $(Wolfssl_Enclave_C_Flags) -Xclang -load -Xclang $(SGXSan_Pass) -c $< -o $@
#	@$(CLANG) -Wstrict-prototypes $(Wolfssl_Enclave_C_Flags) -Xclang -load -Xclang $(SGXSan_Pass) -S -emit-llvm -c $< -o ${@:%.o=%.ll}
	@echo "CC   <=  $<"

Wolfssl_Enclave.so: trusted/Wolfssl_Enclave_t.o $(Wolfssl_Enclave_C_Objects) $(LibSGXSanRTEnclave)
	@$(CXX) $(filter-out $(LibSGXSanRTEnclave), $^) $(SGXSanRTEnclave_Link_Flags) -o $@ $(Wolfssl_Enclave_Link_Flags)
	@echo "LINK =>  $@"
